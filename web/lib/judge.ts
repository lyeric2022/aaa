...
// The Python bridge (agents/web_bridge.py) sends a structured uAgent request to
// the real Judge uAgent. Override with JUDGE_BRIDGE_URL if you run it on a
// different host/port.
 ...
 * coaching. The bridge accepts 0-100 values (the Judge normalizes to 0-1), so
 * we pass the web MoveStats through directly. `power` / `deployability` are
